...
logger = get_logger("coarse_fine_relation", logs_dir=logs_dir)
...
def main():
    ...
    logger.info("Starting coarse_fine_relation analysis...")
    ...
    # Save results
    logger.info("Saving coarse_fine_relation analysis results...")
    output_dir = project_root / cfg.out.results
    output_dir.mkdir(parents=True, exist_ok=True)
    results_file = output_dir / "analysis/coarse_fine_relation_analysis.json"
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    logger.info(f"Results saved to {results_file}")

    logger.info("\n" + "="*60)
    logger.info("Coarse-fine relation analysis complete!")
    logger.info("="*60)
