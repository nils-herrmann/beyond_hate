...
def evaluate_model(cfg, model_id, task_name, logger, project_root, test_ds, SYSTEM_TEXT, USER_TEXT):
    ...
    for i in tqdm(range(0, len(test_dataset_converted), batch_size), desc="Evaluating batches"):
        ...
        for data_id, labels, decoded_output in zip(data_ids, labels_list, decoded_outputs):
            ...
            pred_incivility, pred_intolerance = extract_multi_labels(output)
            results.append({
                "id": data_id,
    ...
    avg_accuracy = (evaluation["incivility"]["accuracy"] + evaluation["intolerance"]["accuracy"]) / 2
    avg_f1 = (evaluation["incivility"]["f1_score"] + evaluation["intolerance"]["f1_score"]) / 2
    avg_invalid_prediction_rate = (evaluation["incivility"]["invalid_prediction_rate"] + evaluation["intolerance"]["invalid_prediction_rate"]) / 2
    ...
    for label in ["incivility", "intolerance"]:
        wandb.log({
    ...
    out_path = Path(cfg.out.results) / "fine"
